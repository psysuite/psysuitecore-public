...
//                    Log.d("TestBasic", "visual: type=${vm.type}, onset=$v, elapsed=$elapsedms")
    ...
    // ---------------------------------------------------------------------------------------------
    fun deliverAlignedStimulus(type:Int,
                                         onEnd:()-> Unit = {}){
    ...
    fun deliverUnimodalStimulus(type:Int, onEnd:() -> Unit = {}){
        when(type){
            STIM_TYPE_A1 -> deliverA1Stimulus(onEnd)
            STIM_TYPE_A2 -> deliverA2Stimulus(onEnd)
            STIM_TYPE_A3 -> deliverA3Stimulus(onEnd)
            STIM_TYPE_T1, STIM_TYPE_T2 -> deliverTStimulus(onEnd)
            STIM_TYPE_V1, STIM_TYPE_V2 -> deliverVStimulus(onEnd)
        }
    }

    fun deliverA1Stimulus(onEnd:() -> Unit = {}){

        try {
            if(mAudioManager == null)   throw Exception("deliverA1Stimulus: mAudioManager is null")
            if(!mAudioManager.isValid)  throw Exception("deliverA1Stimulus: mAudioManager is not valid")
            if(mAudioManager.type != STIM_TYPE_A1)  throw Exception("deliverA1Stimulus: mAudioManager type is not consistent")
    ...
    private fun deliverA2Stimulus(onEnd:() -> Unit = {}){

        try {
            if(mAudioManager == null)   throw Exception("deliverA2Stimulus: mAudioManager is null")
            if(!mAudioManager.isValid)  throw Exception("deliverA2Stimulus: mAudioManager is not valid")
            if(mAudioManager.type != STIM_TYPE_A2)  throw Exception("deliverA2Stimulus: mAudioManager type is not consistent")
    ...
    private fun deliverA3Stimulus(onEnd:() -> Unit = {}){

        try {
            if(mAudioManager == null)   throw Exception("deliverA3Stimulus: mAudioManager is null")
            if(!mAudioManager.isValid)  throw Exception("deliverA3Stimulus: mAudioManager is not valid")
            if(mAudioManager.type != STIM_TYPE_A3)  throw Exception("deliverA3Stimulus: mAudioManager type is not consistent")
    ...
    fun deliverTStimulus(onEnd:() -> Unit = {}){
        try {
            if(mTactileManager == null)   throw Exception("deliverA2Stimulus: mTactileManager is null")
            if(!mTactileManager.isValid)  throw Exception("deliverA2Stimulus: mTactileManager is not valid")
